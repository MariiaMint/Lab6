...
public class CommandExecutorClient {
    public void add() {
        sendMessWithHuman("add", HumanBeing.creatingHuman(scanner));
    ...
    public static void update(HumanBeing person) {
        ...
        sendMessWithHuman("update", person);
        serverRead();
        print(serverRead().getMessageName());
    }
    ...
    public void countLessThanImpactSpeed(String strIs) {
        ...
        sendMessWithArg("count_less_than_impact_speed", strIs);
        print(serverRead().getMessageName());
    }
    ...
    public void executeScript(String arg) {
        ...
        if (listCommands.size() == 0) {
            sendMessWithArg("execute_script", "0");
        } else {
            filePaths.add(arg);
            int m = -1;
            int i = -1;
            for (String command : listCommands) {
                ...
                if (st[0].equals("add")) {
                    ...
                    try {
                        HumanBeing human = humanForScript(listCommands, i);
                        sendMessWithHuman("add_if_max", human);
                    } catch (NumberFormatException e) {
                        print("В файле есть некорректные данные");
                        break;
                    }
...
//                }
                    ...
                    if (st.length == 2 && st[1].chars().allMatch(Character::isDigit)) {
                        try {
                            HumanBeing human = humanForScript(listCommands, i);
                            sendMessWithArg("remove_by_id", st[1]);
